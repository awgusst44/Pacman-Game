...
2. Creaza fișiere hpp cu definirea structurilor de bază necesitate pentru proiect.  1. motor de joaca  GameEngine.hpp 2. desenator Renderer.hpp 3. ascultator  Listener.hpp.
   *A creat toate fișierele corespunzătoare respectînd criteriile de notare pentru c++.
3. Te rog sa mi explici ce presupune fiecare script.
   *Mi-a explicat ce presupune fișierele Types.hpp , Listener.hpp , Renderer.hpp si Gameengine.hpp
4. E posibil sa dau run la acest joc?
   * Mi-a spus că nu pentru că nu sunt implementate fișierele cpp și (main) .
